...
3. **Запуск SQL прокси-сервера:**  
    ...
    * `<log_file_path>` - путь к файлу относительно корневой директории проекта для логирования запросов  (например, `logs/sql.logs`)
...
```bash 
./../bin/ProxyServer 127.0.0.1 5002 127.0.0.1 5432 logs/sql.logs
```
