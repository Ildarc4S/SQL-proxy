...
void runClient(int client_id, int time, const std::string& db_name, const std::string& db_user,
               const std::string& db_password, const std::string& db_host, const std::string& db_port, const std::string& query) {
    try {
        ...
        while (std::chrono::steady_clock::now() < end_time) {
            std::string command = "psql \"host=" + db_host + " port=" + db_port + 
...
int main(int argc, char* argv[]) {
    if (argc != 9) {
        std::cerr << "Usage: " << argv[0] << " <db_name> <db_user> <db_password> <db_host> <db_port> <db_query> <time_seconds> <clients_count>" << std::endl;
        return 1;
    }
